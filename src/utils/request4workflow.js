...
// 创建 axios 实例
const request4workflow = axios.create({
  // API 请求的默认前缀
  baseURL: '/diit-project',
  timeout: 6000 // 请求超时时间
})
...
// request interceptor
request4workflow.interceptors.request.use(config => {
  const token = storage.get(ACCESS_TOKEN)
...
request4workflow.interceptors.response.use((response) => {
...
const installer = {
  vm: {},
  install (Vue) {
    Vue.use(VueAxios, request4workflow)
  }
}

export default request4workflow

export {
  installer as VueAxios,
  request4workflow as axios
}
