...
// 创建 axios 实例
const request4project = axios.create({
  // API 请求的默认前缀
...
// request interceptor
request4project.interceptors.request.use(config => {
  const token = storage.get(ACCESS_TOKEN)
...
request4project.interceptors.response.use((response) => {
...
const installer = {
  vm: {},
  install (Vue) {
    Vue.use(VueAxios, request4project)
  }
}

export default request4project

export {
  installer as VueAxios,
  request4project as axios
}
